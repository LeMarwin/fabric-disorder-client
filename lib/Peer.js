...
class Peer extends Remote {
	...
	/**
	 * Send an envelope with a transaction to an endorser. This is used to commit a transaction to the ledger
	 *
   * @param {byte[]} envelope - Byte data to be included in the broadcast.
	 ...
	 *        that contains a [Transaction]{@link https://github.com/hyperledger/fabric/blob/v1.0.0/protos/peer/transaction.proto#L70}
	 *        in the <code>payload.data</code> property of the envelope.
	 * @param {Number} timeout - A number indicating milliseconds to wait on the
	 *        response before rejecting the promise with a timeout error. This
	 *        overrides the default timeout of the Peer instance and the global
	 *        timeout in the config settings.
	 * @returns {Promise} A Promise for a {@link BlockRecipe}
	 */

	async sendEnvelope(envelope, timeout) {
		const method = 'sendProposal';
		logger.debug('%s - Start ----%s %s', method, this.getName(), this.getUrl());
		const self = this;
		let rto = self._request_timeout;
		...
		if (!envelope) {
			throw new Error('Missing envelope to send to peer');
		}
		...
		return new Promise((resolve, reject) => {
			...
			self._endorserClient.sendEnvelope(envelope, (err, blockRecipe) => {
				clearTimeout(send_timeout);
				if (err) {
					logger.debug('%s - Received block recipe from: %s status: %s', method, self._url, err);
					if (err instanceof Error) {
						reject(err);
					} else {
						reject(new Error(err));
					...
					if (blockRecipe) {
						logger.debug('%s - Received block recipe from peer "%s": status - %s', method, self._url, (blockRecipe.response &&  blockRecipe.response.status) ? blockRecipe.response.status : 'undefined');
						// 400 is the error threshold level, anything below that the endorser will endorse it.
						if (blockRecipe.response && blockRecipe.response.status < 400) {
							blockRecipe.peer = self.getCharacteristics();
							resolve(blockRecipe);
						} else if (blockRecipe.response && blockRecipe.response.message) {
							const error = Object.assign(new Error(blockRecipe.response.message), blockRecipe.response);
							error.peer = self.getCharacteristics();
							error.isBlockRecipe = true;
							reject(error);
						} else {
							const return_error = new Error(util.format('GRPC client failed to get a proper response from the peer "%s".', self._url));
							return_error.peer = self.getCharacteristics();
