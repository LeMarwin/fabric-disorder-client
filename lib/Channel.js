...
const Channel = class {
	...
	async sendUnorderedTransaction(request, timeout) {
		logger.debug('sendTransaction - start :: channel %s', this);
		...
		const envelope = Channel.buildEnvelope(this._clientContext, chaincodeProposal, endorsements, proposalResponse, use_admin_signer);
		const reqs = Array.from(this._channel_peers.values()).map(chPeer => chPeer._peer.sendEnvelope(envelope, timeout));
		const allSettled = require('promise.allsettled');
		return allSettled(reqs);
	}
